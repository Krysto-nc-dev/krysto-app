...
import { useCreatePlasticTypeMutation, useDeletePlasticTypeMutation, useGetPlasticTypesQuery } from '../../slices/plasticTypesSlice';
...
const AdminPlasticsScreen = () => {
  const { data: plasticTypes, error: plasticTypesError, isLoading: plasticTypesLoading , refetch } = useGetPlasticTypesQuery();
          ...
          <div key={plastic._id} className="bg-white shadow-md rounded-lg overflow-hidden">
            ...
            <div className="p-4">
              ...
              <div className='mb-4 flex justify-center items-center gap-5' >
                <Button icon={EyeIcon}>
                  <Link to={`/admin/plastic-type-details/${plastic._id}`}>Voir</Link>
                </Button>
                <Button version={"warning"} icon={Edit}>
                  <Link to={`/admin/plastic-type-modifier/${plastic._id}`}>Editer</Link>
                </Button>
                <Button version={"danger"} icon={Trash}>
                  <Link to={`/admin/plastic-type-modifier/${plastic._id}`}>Supprimer</Link>
                </Button>
