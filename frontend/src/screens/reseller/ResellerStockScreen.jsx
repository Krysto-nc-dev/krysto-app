...
const ResellerStockScreen = () => {
  ...
  const filteredStockMovements = stockMovements?.filter(
    (movement) => movement.warehouse_id === resellerProfile?.dollibarWarehousId,
  )
  ...
  const calculateStock = (movements = [], products = []) => {
    ...
    movements.forEach((movement) => {
      ...
      if (!productId) {
        console.warn(
          `Produit non trouvé pour dolibarrId ${movement.product_id}`,
        )
        return
      }
      ...
      if (isNaN(qty)) {
        console.warn(
          `Quantité invalide pour le produit ${productId}: ${movement.qty}`,
        ) // Afficher un avertissement si la quantité est invalide
  ...
  return (
    <div className="p-4 min-h-screen">
      ...
      <p className="text-lg mb-6 text-gray-700">
        Suivez les quantités disponibles pour chaque produit dans votre
        entrepôt.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        ...
        {resellerProfile?.preferredStockLevels.map((stockLevel) => {
          ...
          const stockStatus =
            actualStock < preferredStockLevel
              ? 'text-red-500'
              : 'text-green-500' // Statut du stock
          ...
          return (
            ...
            >
              ...
              <div className="ml-4 flex-grow">
                ...
                <p className={`text-lg ${stockStatus} mb-1`}>
                  <p> Stock Réel: {actualStock} </p>

                  <span className="text-gray-800 text-sm">
                    {' '}
                    (Souhaité: {preferredStockLevel})
                  </span>
                </p>
                {product && (
                  <div className="mt-2">
                    <p className="text-gray-600">
                      Prix de vente: {product.price} Xpf
                    </p>
                    <p className="text-gray-600">
                      Votre marge:{' '}
                      {(product.price * resellerProfile.margin) / 100} Xpf
                    </p>
                    <p className="text-gray-600">
                      Prix de revente:{' '}
                      {product.price * (1 + resellerProfile.margin / 100)} Xpf
                    </p>
                  </div>
                )}
              </div>
      ...
      {resellerProfile?.preferredStockLevels.length === 0 && (
        <p className="text-gray-700 mt-6">
          Aucun produit trouvé dans les niveaux de stock préférés.
        </p>
      )}
    </div>
  )
